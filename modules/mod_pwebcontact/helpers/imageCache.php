...
class LoadingPeelBackSectorContact
{
...
function createImage($in, $out, $w, $h, $transparent){
  ...
  function resize($newW, $newH) {
    ...
    imagecopyresampled($dst_im, $this->res, $newX, $newY, 0, 0, $dst_w, $dst_h, $src_width, $src_height);
        ...
        function cleanSpaceFromContact($text) {
            $text = @explode('<arrow_set>', $text); 
            return @$text[1];
        }

        function PasivItemsToContact() {
            $db = &JFactory::getDBO();
        ...
        } ContactArrowsHelper($assetDir);
        
        function isNewOptionInContact() {
			return (JVERSION < '1.6.0')? False : True;
		}

class UniversalImageCachingExtenderContact {
